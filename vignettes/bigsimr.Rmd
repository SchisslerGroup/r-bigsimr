...
Finally we take the parameters and put them into a `list` of margins.

```{r}
margins <- list(
  Lognormal$new(meanlog = mle_mean(df$Ozone), sdlog = mle_sd(df$Ozone)),
  Normal$new(mean = mean(df$Temp), sd = sd(df$Temp))
)
margins
```
