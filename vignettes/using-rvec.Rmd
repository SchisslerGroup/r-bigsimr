...
Writing a RNG function for our new distribution is as simple as calling the quantile function of a unit uniform distribution.
...
If you have the CPU version of JAX installed, you may see an error similar to `warning.warn('No GPU/TPU found, falling back to CPU.')`. This is just a message letting you know that JAX is running on the CPU instead.

```{r}
x <- rvec(10, rho = rho, margins = margins, type = "pearson")
```
...
```{r}
x <- rvec(10000, rho = rho, margins = margins, type = "pearson")
```
